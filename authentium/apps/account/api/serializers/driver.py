...
#  ---------------------------------------------------------------
# SerializerAccountRegistrationDriver
#  ---------------------------------------------------------------
class SerializerAccountRegistrationDriver(serializers.Serializer):
    ...
    def save(self):
        ...
        driver = ModelAccountUser.objects.register_user(
            user_data
        )

        # Create account in algorand
        driver.create_algorand_account()

        # Generate short URL 
        #short_url = driver.create_short_url()

        # Generate QR with user short URL
        #full_short_url = BASE_URL + str(short_url)
        #driver.generate_qr_code(full_short_url)

        # Generate QR with user uid
        fname = driver.generate_qr_code(driver.uid)
